...
class SyntheticModel(object):
    ...
    def run_mcmc(self, method="dynesty", reload=False, 
        ...
        if method == "dynesty":
            savedir = os.path.join("results_dynesty/", self.config_id)

        elif method == "alabi":
            savedir = os.path.join("results_alabi/", self.config_id)
    ...
    def variance_global_sensitivity(self, param_values=None, Y=None, nsample=1024):
        ...
        savedir = os.path.join("results_sensitivity", self.config_id)
        ...
        sn.heatmap(table_s1, annot=True, annot_kws={"size": 18}) 
        ...
        sn.heatmap(table_sT, annot=True, annot_kws={"size": 18}) 
